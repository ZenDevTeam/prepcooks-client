<template>
  <div>
    <b-button v-b-toggle.collapse-1 variant="primary" class="reviewButton"
    ...
    <b-collapse id="collapse-1" class="mt-2">
      <b-card>
        <div>
          ...
          ></b-form-textarea>
          <pre class="mt-3 mb-0" v-bind:class="{ 'text-danger': hasError }">
Charcters left: {{ remainingCount }}</pre
          >
        </div>
        <b-button class="submitReview" v-on:click="submitComment"
          >Submit Review
        </b-button>
      </b-card>
    </b-collapse>
...
export default {
  ...
  created: function () {
    console.log("name");
    console.log(this.companyCurrentName);
    this.$http({
      method: "post",
      url: API_URL,
      data: {
  ...
  methods: {
    ...
    validateComment() {
      if (
        ...
        this.newComment.content.length > 150 ||
